...
export async function POST(
  ...
  try {
    ...
    if (!user) {
      return formatError(new AuthenticationError('Authentication required to accept connection request'));
    }
    ...
    const connection = await commandService.acceptRequest(context, connectionId);
