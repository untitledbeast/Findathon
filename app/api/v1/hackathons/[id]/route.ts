...
export async function PATCH(
  ...
  try {
    ...
    /*
     * Normalize nullable fields before passing data into the
     * domain/service layer.
     *
     * HackathonDTO.organizer is:
     *   string | undefined
     *
     * but the validation schema may produce:
     *   string | null | undefined
     *
     * Therefore, remove `organizer` when it is null rather than
     * passing null into HackathonCommandService.update().
     */
    const updateData: Partial<HackathonDTO> = {
      ...validatedData,
      organizer: validatedData.organizer ?? undefined
    };
    ...
    const result = await commandService.update(
      context,
      id,
      updateData
    );
