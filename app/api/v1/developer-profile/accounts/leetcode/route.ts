...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!user) {
      return formatError(new AuthenticationError('Authentication required to disconnect LeetCode'));
    }
    ...
    const updated = await commandService.disconnectLeetCode(context);
