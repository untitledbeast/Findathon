...
export function createHackathonCardViewModel(dto: HackathonDTO): HackathonCardViewModel {
  ...
  return {
    id: dto.id,
    title: dto.title || 'Untitled Hackathon',
    ...
    formattedPrize: dto.prizePool || (typeof dto.prizeAmount === 'number' ? `$${dto.prizeAmount.toLocaleString('en-US')}` : 'TBD'),
    statusText,
    statusColor,
    daysRemainingText,
    formattedStartDate,
    ...
    reviewsCountText: `(${dto.reviewsCount || 0} review${(dto.reviewsCount || 0) === 1 ? '' : 's'})`,
