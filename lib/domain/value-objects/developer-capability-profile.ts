...
export interface SkillCapabilityRecord {
  ...
  category: string;
  displayLabel: string;
  proficiency: number; // 0..1 (absolute saturating score)
  confidence: number;  // 0..1 (per-skill evidence robustness)
  evidenceCount: number;
  sources: SkillEvidenceSource[];
  lastObservedAt: number;
  observationState: 'PRESENT' | 'NOT_OBSERVED' | 'UNKNOWN';
}
...
export interface DeveloperCapabilityProfileProps {
  ...
  skillRecords?: Record<string, SkillCapabilityRecord>; // Per-skill capability with confidence & observation state
...
export class DeveloperCapabilityProfile {
  ...
  public get skillRecords(): Record<string, SkillCapabilityRecord> { return { ...(this.props.skillRecords || {}) }; }
  ...
  /**
   * Safe observation lookup: Returns explicit record or NOT_OBSERVED (never assumes incapable).
   */
  public getSkill(id: string): SkillCapabilityRecord {
    if (this.props.skillRecords && this.props.skillRecords[id]) {
      return this.props.skillRecords[id];
    }
    return {
      id,
      category: 'unknown',
  ...
  /**
   * [V1_DETERMINISTIC_BASELINE]
   * Preserved original V1 engine implementation for control and baseline evaluation.
   */
  public static fromEvidenceV1(
    userId: string,
    ...
    for (const evidence of evidenceList) {
      ...
      if (evidence.source === 'github') {
        if (evidence.evidenceType === 'repo') githubRepoCount++;
    ...
    // Relative max-normalization (V1 baseline)
    ...
    if (evidenceList.length > 0) {
      const volumeFactor = Math.min(0.5, evidenceList.length / 20);
      const diversityFactor = sourceCount >= 2 ? 0.35 : 0.20;
      const weightFactor = Math.min(0.15, totalEvidenceWeight / 50);
  ...
  /**
   * [V1_5_ACCURACY_FOUNDATION]
   * Absolute saturating proficiency + diminishing marginal returns per evidence + per-skill confidence.
   */
  public static fromEvidence(
    ...
    // Track diminishing return weights and occurrences per skill
    const skillWeights = new Map<string, number>();
    const skillOccurrences = new Map<string, number>();
    const skillSources = new Map<string, Set<SkillEvidenceSource>>();
    const skillLastSeen = new Map<string, number>();
    const skillMetadata = new Map<string, { category: string; displayLabel: string; parentDomain?: string }>();
    ...
    // Helper: Accumulates evidence with diminishing marginal returns (1 / sqrt(k))
    const addSkillEvidence = (
      ...
      const count = (skillOccurrences.get(skillId) || 0) + 1;
      skillOccurrences.set(skillId, count);

      // Diminishing marginal returns formula: Delta_W = w / sqrt(k)
      const marginalContribution = effectiveWeight / Math.sqrt(count);
      skillWeights.set(skillId, (skillWeights.get(skillId) || 0) + marginalContribution);

      if (!skillSources.has(skillId)) skillSources.set(skillId, new Set<SkillEvidenceSource>());
      skillSources.get(skillId)!.add(source);

      const prevTime = skillLastSeen.get(skillId) || 0;
      if (timestamp > prevTime) skillLastSeen.set(skillId, timestamp);

      if (!skillMetadata.has(skillId)) {
        skillMetadata.set(skillId, { category, displayLabel, parentDomain });
      }
    };
    ...
    for (const evidence of evidenceList) {
      ...
      if (evidence.source === 'github') {
        ...
        if (typeof signals.language === 'string') {
          const canonical = SkillNormalizer.normalize(signals.language);
          if (canonical && canonical.category === 'language') {
            addSkillEvidence(canonical.id, canonical.category, canonical.displayLabel, effectiveWeight, evidence.source, timestamp, canonical.parentDomain);
            if (canonical.parentDomain) {
              addSkillEvidence(canonical.parentDomain, 'domain', canonical.parentDomain, effectiveWeight * 0.6, evidence.source, timestamp);
            }
          }
        }
          ...
          for (const [langKey, bytes] of Object.entries(signals.languages)) {
            ...
            if (canonical && canonical.category === 'language') {
              const byteScale = typeof bytes === 'number' && bytes > 0
                ? Math.min(1.5, Math.max(0.5, Math.log10(bytes) / 4))
                : 1.0;
              addSkillEvidence(canonical.id, canonical.category, canonical.displayLabel, effectiveWeight * byteScale, evidence.source, timestamp, canonical.parentDomain);
            }
          }
        }
          ...
          for (const tag of signals.topics) {
            ...
            if (canonical) {
              if (canonical.category === 'framework') {
                addSkillEvidence(canonical.id, canonical.category, canonical.displayLabel, effectiveWeight, evidence.source, timestamp, canonical.parentDomain);
                if (canonical.parentDomain) {
                  addSkillEvidence(canonical.parentDomain, 'domain', canonical.parentDomain, effectiveWeight * 0.7, evidence.source, timestamp);
                }
              } else if (canonical.category === 'domain') {
                addSkillEvidence(canonical.id, canonical.category, canonical.displayLabel, effectiveWeight, evidence.source, timestamp);
              } else if (canonical.category === 'language') {
                addSkillEvidence(canonical.id, canonical.category, canonical.displayLabel, effectiveWeight * 0.5, evidence.source, timestamp, canonical.parentDomain);
              } else if (canonical.category === 'skill' || canonical.category === 'database' || canonical.category === 'cloud_devops') {
                addSkillEvidence(canonical.id, canonical.category, canonical.displayLabel, effectiveWeight, evidence.source, timestamp, canonical.parentDomain);
              }
            }
      ...
      if (evidence.source === 'leetcode') {
        if (evidence.evidenceType === 'activity') {
          ...
          if (signals.languageName && typeof signals.problemsSolved === 'number') {
            ...
            if (canonical && canonical.category === 'language') {
              const countScale = Math.min(1.5, Math.max(0.5, Math.log10(signals.problemsSolved + 1)));
              addSkillEvidence(canonical.id, canonical.category, canonical.displayLabel, effectiveWeight * countScale, evidence.source, timestamp, canonical.parentDomain);
            }
          }
        }
        ...
        if (evidence.evidenceType === 'submission' && signals.tagSlug) {
          ...
          if (canonical) {
            if (canonical.id === 'skill.dsa' || canonical.id === 'skill.problem_solving') {
              addSkillEvidence('domain.backend', 'domain', 'Backend & Cloud', effectiveWeight, evidence.source, timestamp);
            } else if (canonical.category === 'domain') {
              addSkillEvidence(canonical.id, canonical.category, canonical.displayLabel, effectiveWeight, evidence.source, timestamp);
            }
          }
    ...
    /**
     * Calculates absolute, bounded, saturating capability proficiency (GATE 1).
     * Replaces dangerous relative max-normalization where 1 tiny repo yielded 1.0.
     * 
     * Evidence saturation model:
     * - 1 tiny repo (w=0.2): P = 1 - e^(-0.2/0.8) ≈ 0.221 (bounded, realistic)
     * - 1 medium repo (w=0.5): P = 1 - e^(-0.5/0.8) ≈ 0.465
     * - 1 substantial repo (w=1.0): P = 1 - e^(-1.0/0.8) ≈ 0.713
     * - 3 substantial repos (w=2.28): P = 1 - e^(-2.28/0.8) ≈ 0.942
     */
    const calculateProficiency = (weight: number): number => {
      if (weight <= 0) return 0;
      const K_SAT = 0.8;
      const saturatingProficiency = 1.0 - Math.exp(-weight / K_SAT);
      return Math.max(0.0, Math.min(1.0, Math.round(saturatingProficiency * 1000) / 1000));
    };

    // Per-Skill Confidence Calculator
    const computeSkillConfidence = (count: number, sources: Set<SkillEvidenceSource>, w: number): number => {
      if (count === 0) return 0;
      const vol = Math.min(0.50, count / 4);
      const div = sources.size >= 2 ? 0.30 : 0.15;
      const wt = Math.min(0.20, w / 5);
      return Math.min(1.0, Math.round((vol + div + wt) * 100) / 100);
    };
    ...
    const domains: Record<string, number> = {};
    const skills: Record<string, number> = {};
    const skillRecords: Record<string, SkillCapabilityRecord> = {};

    for (const [skillId, w] of skillWeights.entries()) {
      const meta = skillMetadata.get(skillId) || { category: 'skill', displayLabel: skillId };
      const prof = calculateProficiency(w);
      const conf = computeSkillConfidence(
        skillOccurrences.get(skillId) || 0,
        skillSources.get(skillId) || new Set(),
        w
      );
      ...
      skillRecords[skillId] = {
        ...
        proficiency: prof,
        confidence: conf,
        evidenceCount: skillOccurrences.get(skillId) || 0,
        sources: Array.from(skillSources.get(skillId) || []),
        lastObservedAt: skillLastSeen.get(skillId) || now,
        observationState: 'PRESENT'
      };

      if (meta.category === 'language') languages[skillId] = prof;
      else if (meta.category === 'framework') frameworks[skillId] = prof;
      else if (meta.category === 'domain') domains[skillId] = prof;
      else skills[skillId] = prof;
    }
    ...
    if (!profileEntity?.experienceLevel) {
      if (
        (githubRepoCount >= 5 && Object.keys(languages).length >= 3 && totalEvidenceWeight > 30) ||
        (leetcodeTotalSolved >= 300 && leetcodeMediumHard >= 100) ||
        (leetcodeContestRating >= 1800)
      ) {
        technicalLevel = 'advanced';
  ...
  public toJSON(): DeveloperCapabilityProfileProps {
    return {
      ...
      skillRecords: this.props.skillRecords ? { ...this.props.skillRecords } : undefined,
